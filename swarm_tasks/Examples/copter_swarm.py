...
same_height=30
different_height=[30,35,40,45,50,55,60,65,70,75]
...
heartbeat_ip=["192.168.6.151","192.168.6.152","192.168.6.153","192.168.6.154","192.168.6.155"]
...
def vehicle_collision_moniter_receive():	
        ...
        while 1:
        	...
        	if decoded_index.startswith("master"):
        		m,master_num=decoded_index.split("-")
        		print("m,master_num",m,master_num)
        		msg='Drone 1 master_num '+str(master_num)+' data received'
        		if(int(master_num)==1):
        			if(master_flag):
        				pass
        			else:
...
slave_heal_ip=["192.168.6.153"]*num_bots
...
def home_monitor_thread():
    ...
    while True:
        try:
            ...
            if not home_pos or len(pos_array) != len(home_pos) or len(pos_array) == len(home_pos)::
	...
	try:            
					...
					for i,b in enumerate(s.swarm):						
						...
						if master_flag:
							...
							if same_alt_flag:
								...
								point1 = LocationGlobalRelative(lat,lon,different_height[i])
							vehicles[i].simple_goto(point1)							
					if(multiple_goals==[]) and (elapsed_time>5):
						disperse_flag=False
						search_flag=True
						break
		...
		if(data.startswith(b"split")) or (split_flag) or (data.startswith(b"specificsplit")):
			...
			if not previous_task_flag:
				...
				if split_flag_val==0 and data.startswith(b"split"):
					split_flag_val+=1
					csv_file_paths=[]
					for i in range(1,len(pos_array)+1):
				...
				if split_flag_val==0 and data.startswith(b"specificsplit"):
					split_flag_val+=1
					csv_file_paths=[]
