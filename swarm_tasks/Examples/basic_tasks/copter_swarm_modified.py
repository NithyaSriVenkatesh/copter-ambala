...
import locatePosition, netifaces, wmi
import math
...
print("read_origin_path", read_origin_path)
# # Example use
...
if file_name == "chandigarh":
    origin = (30.351921, 76.852759)
...
def vehicle_collision_moniter_receive():
    ...
    while 1:
        ...
        if decoded_index.startswith("origin"):
            print(" oRigin!!!!!!!!!!", decoded_index)
            c, o_lat, o_lon = decoded_index.split(",")
...
ip = get_interface_mapping()  # "192.168.6.220    "
print("ip", ip)
...
robot_positions = [([0, 0]) for _ in range(10)]
print("origin#########", origin)
    ...
    try:
        ...
        if data.startswith(b"add"):
            try:
                ...
                try:
                    ...
                    globals()[f"vehicle{sys_id}"] = connect(
                        connection_str, baud=115200, heartbeat_timeout=30
                    )
                    ...
                    home_lock()
                    try:
                        different_height[num_bots - 1] = (
                            different_height[num_bots - 2] + height_difference
                        )
                    except:
        ...
        if (data.startswith(b"search")) or (search_flag):
            ...
            if not previous_task_flag:
                ...
                if search_flag_val == 0:
                    ...
                    for i in range(1, len(pos_array) + 1):
                        csv_file_paths.append(
                            os.path.join(cwd, "searchgrid", f"d{i}.csv")
                        )
                ...
                print("grid_path_array", grid_path_array, search_step)
                removed_uav_grid = []
                removed_grid_path_length = []
                removed_numlines = []
                uncovered_area_points = []
            ...
            while 1:
                ...
                if remove_bot_flag:
                    print(
                        "remove_bot_flag,remove_bot_array",
                        remove_bot_flag,
                        remove_bot_array,
                    )
                    for m in remove_bot_array:
                        ...
                        removed_numlines.append(num_lines.pop(m))
                    remove_bot_array = []
                    remove_bot_flag = False
                    print(
                ...
                for i, b in enumerate(s.swarm):
                    ...
                    if removed_grid_path_array_flag:
                        ...
                        if grid_path_array[i] < num_lines[i]:
                            ...
                            if master_flag and simulation_flag == 1:
                                uav_home_pos = []
                                for vehicle in vehicles:
                                    lat = vehicle.location.global_relative_frame.lat
                                    lon = vehicle.location.global_relative_frame.lon
        ...
        if (
            ...
            if not previous_task_flag:
                ...
                if (
                    split_flag_val == 0
                    and data.startswith(b"split")
                    or data.startswith(b"polyautosplit")
                ):
                    split_flag_val += 1
                    csv_file_paths = []
                    for i in range(1, len(pos_array) + 1):
                        csv_file_paths.append(
                            os.path.join(cwd, "group_split", f"grid_{i}.csv")
                        )
                        reader = csv.reader(open(csv_file_paths[i - 1]))
                        num_lines[i - 1] = len(list(reader))
                    print("csv_file_paths", csv_file_paths, num_lines)
                if (
                    split_flag_val == 0
                    and data.startswith(b"specificsplit")
                    ...
                    for i, e in enumerate(pos_array):
                        csv_file_paths.append(
                            os.path.join(cwd, "group_split", f"grid_{e}.csv")
                        )
                        reader = csv.reader(open(csv_file_paths[e - 1]))
                        num_lines[e - 1] = len(list(reader))
                    print("csv_file_paths", csv_file_paths, num_lines)
                # removed_grid_path_array_index=0
                print("grid_path_array", grid_path_array)
                removed_uav_grid = []
            ...
            while 1:
                ...
                for i, b in enumerate(s.swarm):
                    ...
                    # if all(x >= int(num_lines[r]) for r,x in enumerate(grid_path_array)) and not removed_grid_path_length!=[]:
                    finished_drones = [
                        i
                    ...
                    if (removed_grid_path_array_flag) and grid_path_array[i] >= int(
                        ...
                        if removed_grid_path_array_start_val[i] == 0:
                            checkall_removed_grid_path_array_start_val[i] = 1
                            # print("checkall_removed_grid_path_array_start_val",i,checkall_removed_grid_path_array_start_val)
                            continue
                        if (
                            removed_grid_path_array_start_val[i]
                            == removed_grid_path_array[i][1]
