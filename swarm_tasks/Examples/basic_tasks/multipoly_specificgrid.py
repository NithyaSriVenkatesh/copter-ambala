...
class PolygonSpecificSplit:
    ...
    def __init__(self, polygon_latlon_list, origin_gps, endDistance,
                 ...
                 obstacles_latlon_list=None, drone_assignments=None):
        ...
        if isinstance(grid_spacing, (list, tuple)):
            if len(grid_spacing) != len(polygon_latlon_list):
                raise ValueError("grid_spacing list length must match number of polygons")
            self.grid_spacing = grid_spacing
        else:
        ...
        if drone_assignments:
            ...
            if assigned_ids != expected_ids:
                raise ValueError(f"Drone IDs assigned ({all_assigned_ids}) do not match expected drone IDs {list(expected_ids)}")
    ...
    def generate_paths(self):
        ...
        for polygon_idx, (poly_coords, obstacles, drone_ids) in enumerate(
                zip(self.polygon_latlon_list, self.obstacles_latlon_list, self.drone_assignments)):

            # Prepare planner (replace with actual call)
            outer_poly_img, obstacles_img, buffered_polygon, rotated_polygon, rtf = self.create_planner(
                poly_coords, obstacles, len(drone_ids))
