...
import csv
import os
from shapely.geometry import Polygon, MultiPolygon, box, LineString, MultiLineString, GeometryCollection, Point
...
class PolygonAutoSplit:
    ...
    def __init__(self, polygon_latlon_list, origin_gps, endDistance,
                 num_drones=1, grid_spacing=5.0, rotation_angle=90,
        ...
        self.obstacles_latlon_list = obstacles_latlon_list if obstacles_latlon_list else [[] for _ in polygon_latlon_list]
        ...
        self.drone_assignments = self.split_drones_among_polygons(self.num_drones, len(self.polygon_latlon_list))
    ...
    def create_planner(self, polygon_latlon, obstacles_latlon, num_drones):
        ...
        obstacles_img = [self.gps_to_image_coords(obst) for obst in obstacles_latlon] if obstacles_latlon else []
    ...
    def generate_paths(self):
        self.drone_paths = []
        drone_id = 1
        for i, (poly_coords, obstacles, num_drones) in enumerate(zip(self.polygon_latlon_list, self.obstacles_latlon_list, self.drone_assignments)):
            outer_poly_img, obstacles_img, buffered_polygon, rotated_polygon, rtf = self.create_planner(poly_coords, obstacles, num_drones)
    ...
    def save_paths(self):
        if not self.drone_paths:
            raise RuntimeError("No drone paths generated yet. Call generate_paths() first.")
        ...
        for path in self.drone_paths:
            ...
            kml_path = os.path.join(self.output_dir, f"poly{polygon_idx}_drone{drone_id}.kml")
            kml.save(kml_path)
    ...
    def visualize(self):
        ...
        for poly_coords, obstacles_latlon in zip(self.polygon_latlon_list, self.obstacles_latlon_list):
