...
async def vehicle_connection():
    ...
    async def connect_vehicle(index, port):
        nonlocal num_bots
        try:
            vehicle = await asyncio.to_thread(
                ...
                f'udpin:{ip}:{port}',
                baud=115200,
                heartbeat_timeout=heartbeat_ip_timeout[index]
            )
            print(f'Drone{index + 1}')
            vehicles.append(vehicle)
            pos_array.append(vehicle._master.target_system)
            num_bots += 1
            print(f"Drone{index + 1} Connected")
        except Exception as e:
            print(f"Vehicle {index + 1} is lost: {e}")
