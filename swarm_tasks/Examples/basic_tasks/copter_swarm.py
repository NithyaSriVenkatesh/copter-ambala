				...
				for i,b in enumerate(s.swarm):
					if(len(checkall_removed_grid_path_array_start_val)==len(pos_array)):
					    if all(c==1 for c in checkall_removed_grid_path_array_start_val):
						    ...
						    checkall_removed_grid_path_array_start_val=[0]*len(pos_array)
						    landing_flag=True
					if any(x >= int(num_lines[r]) for r,x in enumerate(grid_path_array)) and removed_grid_path_length!=[] and not removed_grid_path_array_flag:						
						print("removed_grid_path_length",removed_grid_path_length,removed_numlines)						
						allocation,remaining_points_list  = allocate_drones(removed_numlines, removed_grid_path_length, len(pos_array))
						...
						for x,v in enumerate(remaining_points_list):
						    ...
						    print("JJJ",allocation[x],removed_grid_path_length[x],int(num_lines[i]))
						    ...
						    print("remanining",remaining)
							# Sort areas by remaining points (ascending)
						    min_sorted = sorted(enumerate(remaining), key=lambda x: x[1])
						    print("Areas sorted by remaining points:", min_sorted)
						    for m in range(allocation[x]):
						        if(m!=0):
						            end_index+=add_points
						        if(end_index>int(num_lines[m])):
						            end_index=int(num_lines[m])

						        removed_grid_path_array[min_sorted[m][0]] = (start_index, end_index)
						        removed_grid_path_array_start_val[min_sorted[m][0]] = start_index
						        removed_grid_filename[min_sorted[m][0]]=removed_uav_grid[x]
		...
		if data.startswith((b"split", b"specificsplit", b"polyspecificsplit", b"polyautosplit")) or split_flag:
			...
			while 1:
				...
				for i,b in enumerate(s.swarm):
					...
					if any(	x >= int(num_lines[i]) for x in grid_path_array) and removed_grid_path_length!=[] and not removed_grid_path_array_flag:						
